...
class SsaReadPosition extends TSsaReadPosition {
  ...
  /**
   * Holds if `guard` directly controls this position with the value
   * `testIsTrue`.
   */
  predicate isControledBy(Guard guard, boolean testIsTrue) {
    exists(BooleanValue b | b.getValue() = testIsTrue |
      guard.controlsNode(this.(SsaReadPositionBlock).getBlock().getANode(), _, b)
    )
  }
}
